...
public class CameraHelper implements Camera.PreviewCallback {
    ...
    private Camera.Size getBestSupportedSize(List<Camera.Size> sizes, Point previewViewSize) {
        ...
        double targetRatio = ((double) previewViewSize.x / previewViewSize.y);  //目标大小的宽高比
        double minDiff = targetRatio;
        for (int i = 0; i < sizes.size(); i++) {
            Camera.Size ones = sizes.get(i);
            if (ones.width == previewViewSize.x && ones.height == previewViewSize.y) {
                bestSize = ones;
                break;
            }
            double supportedRatio = ((double) ones.width / ones.height);
            if (Math.abs(supportedRatio - targetRatio) <= minDiff) {
                minDiff = Math.abs(supportedRatio - targetRatio);
                if (ones.width > previewViewSize.x) {
                    bestSize = ones;
                }
            }
        }
